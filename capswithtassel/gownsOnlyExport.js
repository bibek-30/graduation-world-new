...
fs.createReadStream(filePath)
  ...
  .on("data", (data) => {
    ...
    if (
      data.name != "" &&
      data.visibility == 4 &&
      data._attribute_set == "cap with tassel"
    ) {
      push = true;
      productCount++;
      ...
      productsObj[productCount]["description"] = data.short_description
        .replace(/(\r\n|\n|\r)/gm, " ")
        .replace(/,/g, ";");
      const secondFullStopIndex = (data.short_description + ".").indexOf(
        ".",
        (data.short_description + ".").indexOf(".") + 1
      );

      productsObj[productCount]["meta_desc"] = data.meta_description
        ? data.meta_description
        : data.short_description
            .substring(0, secondFullStopIndex)
            .trim()
            .replace(/<[^>]*>/g, "")
            .replace(/(\r\n|\n|\r)/gm, " ")
            .replace(/,/g, ";");
      ...
      if (data._media_image != "") {
        ...
        productsObj[productCount]["images"].push(data.image);
      }
    } else {
      count++;
  ...
  .on("end", () => {
    ...
    console.log(productsObj.description);
      ...
      if (productsObj.hasOwnProperty(key)) {
        ...
        if (sizeValues.length > 0) {
          productss.push({
            ...
            size: sizeValues[0],
            image:
              "https://cdn.shopify.com/s/files/1/0780/4793/6800/files/" +
              imagesValues[0].split("/").pop(),
            price: productObj.price,
            handle: productObj.handle,
            description: productObj.description,
            meta_desc: productObj.meta_desc.replace(/,/gm, ";"),
            meta_title: productObj.meta_title,
            vendor: "graduationworld",
            tag: "caps_gown_tassel_set",
          ...
          slicedheightxsizeValues.forEach((heightXsize, index) =>
            productss.push({
              ...
              image:
                "https://cdn.shopify.com/s/files/1/0780/4793/6800/files/" +
                imagesValues[index + 1]?.split("/").pop(),
              handle: productObj.handle,
              price: productObj.price,
              description: "",
              varaint_sku:
                productObj.handle +
                "-" +
                slicedheightxsizeValues[index] +
                "-" +
                slicedSizeValues[index],
              meta_desc: "",
              meta_title: "",
              vendor: "",
              tag: "",
          ...
          productss.push({
            ...
            image:
              "https://cdn.shopify.com/s/files/1/0780/4793/6800/files/" +
              slicedImagesValues[0].split("/").pop(),
            price: productObj.price,
            handle: productObj.handle,
            description: productObj.description,
            meta_desc: productObj.meta_desc.replace(/,/gm, ";"),
